...
public class FactorialServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		req.getRequestDispatcher("/WEB-INF/views/04/factorial.jsp").forward(req, resp); //redirection을 사용할 경우에는 클래스패스 절대경로가 필요하다.
		//forward와 include
	}
		...
		String param = request.getParameter("operand");
		   ...
		   if(param != null && !param.isEmpty()){   // null체크 하나만으로는 데이터가 비어있는지 확인할 수 없다. (화이트스페이스때문)
		      try{
		         ...
		         long result = factorial(operand);   // catch에 걸리지 않은 경우(제대로 넘어온 경우)
		   ...
		   request.getRequestDispatcher("/WEB-INF/views/04/factorial.jsp").forward(request, response); //redirection을 사용할 경우에는 클래스패스 절대경로가 필요하다.
