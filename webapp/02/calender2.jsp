...
<body>
	...
	<table>
		...
		<caption>
		<button type="button" onclick="location='calender2.jsp?year=<%=bef_year %>&month=<%=bef_month%>'">이전</button> <!-- location => jsp로 넘어가겠다. -->
		<%=year%>년	<%=month + 1%>월	
		<button type="button" onclick="location='calender2.jsp?year=<%=aft_year %>&month=<%=aft_month%>'">다음</button>
		</caption>
